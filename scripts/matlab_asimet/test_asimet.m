...
function test_asimet(deployment,d_file)
% test_asimet((deployment,d_file)
% i.e. deployment = 202008
% d_file = 20200825
% test_asimet(deployment,d_file)
% 
close all; 
[yr,mo,da]=datevec(now);
disp(['Begin M1 Asimet testing ',datestr(now)])
clear mo da

% for running on the virtual machine bogdata:
% path_prefix = '/mbari'
% for running locally on a mac (must have loaded ssdsdata):
path_prefix = '/Volumes'
