...
potu_filt <- left_join(samp_500, potu_filt)
...
filename = paste(directory, marker,'_Lipolagus_casts_throughtime_nohake_noanch.png', sep='')
print(var)
filename
...
test %>%
  ...
  labs(x='depth (m)', y='Percent Total Reads', title="Lipolagus by Cast") +
  #day night transition
  scale_fill_manual(values = c("darkgoldenrod1", "deepskyblue", "chartreuse2"))

filename = paste(directory, marker,'_Lipolagus_bycast_nohake_noanch.png', sep='')
print(var)
filename
