...
dna_stats <- stats %>%
  ...
  pivot_wider(id_cols = depth_bin2,names_from = diel, values_from = mean ) %>%
  mutate(DNA_diff = day-night) %>%
  mutate(depth = case_when(depth_bin2 == "0-100m" ~ 0,
                          depth_bin2 == "100-200m" ~ 100,
                          depth_bin2 == "200-300m" ~ 200,
                          depth_bin2 == "300-400m" ~ 300,
                          depth_bin2 == "400-500m" ~ 400,
                          depth_bin2 == "500-600m" ~ 500,
                          depth_bin2 == "600-700m" ~ 600))
st_join <- full_join(ac_diff, dna_stats, by='depth',suffix = c('_acc', '_edna'))

st_join
p <- st_join %>%
  ggplot(aes(x=log_difference, y=DNA_diff, color = depth_bin2)) +
  geom_point()+
  labs(title = '(Day - Night) mean values', y= 'eDNA difference in percent reads',
       x='acoustic log difference', color='depth')
p
filename = paste(plot_directory, marker,'_Day_Night_Mean_Diff.png', sep='')
print(filename)
...
dna_stats <- stats %>%
  filter(Ecological_Category == 'mesopelagic') %>% 
  pivot_wider(id_cols = depth_bin2,names_from = diel, values_from = median ) %>%
  mutate(DNA_diff = day-night) %>%
  mutate(depth = case_when(depth_bin2 == "0-100m" ~ 0,
                           depth_bin2 == "100-200m" ~ 100,
                           depth_bin2 == "200-300m" ~ 200,
...
p <- st_join %>%
  ggplot(aes(x=log_difference, y=DNA_diff, color = depth_bin2)) +
  geom_point()+
  labs(title = '(Day - Night) DNA median values')
p
filename = paste(plot_directory, marker,'_Day_Night_Median_Diff.png', sep='')
print(filename)
ggsave(filename,height = 4, width =4, units = 'in')
