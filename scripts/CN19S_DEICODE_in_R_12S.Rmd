...
p <- pcscores %>%
  ...
  #mutate(mPC2 = mean(PC2)) %>%
  #ungroup() %>%
  ggplot(aes(PC1,PC2,color=ESP)) +
  geom_point(size=3, alpha=0.6, shape=16) #+
  #geom_point(size=7, shape=16, aes(x=mPC1, y=mPC2))

p+labs(x=label.PC1 , y=label.PC2, colour = "Sample Type" )+ ggtitle(marker)+
  #theme_minimal()+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"),
        legend.text = element_text(size = 10, colour = "black"),
...
filename = paste(plot_dir, 'RPCA_',marker,'_PC1PC2_samptype.png', sep='')
filename
ggsave(filename,height = 5, width =7, units = 'in')
