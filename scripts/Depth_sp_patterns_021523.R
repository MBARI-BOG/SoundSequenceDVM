...
# Define days and nights in metadata --------------------------------------
# grouping mechanism for ESP and casts
meta <- samp.c %>% 
...
for (val in taxas) {
  taxa_level = sym(val)
  test <- left_join(potu_filt, meta %>% 
                      select(SampleID, SAMPLING_station,SAMPLING_station_number,
                             depth, local_time, time_label,diel, PlateID, consistent_label),  
                    by = c("SampleID")) %>%
  ...
  center <- test %>%
    ...
    distinct(consistent_label, .keep_all=TRUE) %>%
  ...
  test %>%
    ...
    ggplot(aes(y=depth, x=consistent_label))+
  ...
  filename = paste(directory, marker,'_',taxa_level, '_clabel_throughtime_nohake_noanch_Test.png', sep='')
  print(var)
  filename
  ...
  filename = paste(directory, marker,'_',taxa_level,'_clabel_nohake_noanch_Test.png', sep='')
  print(var)
  filename
